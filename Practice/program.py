...
def gms_window():
    ...
    gms = sg.Window('GMS to Deg', layout, size=(500, 500), resizable=True, finalize=True)
    return gms
...
def office():
    ...
    buttons1 = [[sg.Button('Действие', size=(8, 1), disabled=True, button_color="orange", key="-ENTER-")],
                [sg.Button('Убрать', size=(8, 1), disabled=True, button_color="orange", key='-CLEAR-')],
                [sg.Button('Убрать все', size=(8, 1), disabled=True, button_color="orange", key='-CLEAR ALL-')]]
    buttons2 = [[sg.Button('Удаление файлов', size=(8, 2), disabled=True, button_color='red', key='-DELETE-')],
                [sg.Button('Удалить', size=(8, 1), disabled=True, button_color='red', key='-DELETE IN WINDOW-')]]
